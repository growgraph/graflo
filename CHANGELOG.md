...
- **`gf:APIConnector` and `gf:KafkaConnector`** in `graflo.ttl`, with matching `gf:ApiSource` / `gf:KafkaSource` `BoundSourceKind` individuals and JSON-LD context entries. `APIConnector` and `KafkaConnector` have been part of the `AnyConnector` union for some time; the ontology only ever declared File, Table and Sparql, so two of the five connector kinds had no RDF vocabulary at all.
- **`direction` on the edge read path** — `EdgeDirection` (`OUT` / `IN` / `ANY`) on `Connection.fetch_edges`, implemented on every backend that can answer it. Until now `fetch_edges` was hardcoded outgoing everywhere (`e._from ==`, `-[r]->`, `GO … OVER`), so a logically undirected edge was unreachable from its target endpoint on all eight targets — `directed: false` could not affect a single query. `from_type` / `from_id` now name the **anchor** vertex and `to_type` / `to_id` constrain the vertex at the *other* end, whichever end that is. `default_direction_for_edge(edge)` derives the value from the schema, so `directed: false` in YAML steers the query itself.
  - **ArangoDB** — `ANY` matches either orientation and `IN` anchors on `_to`; the edge index already covered both endpoints, so no branch costs more than the old one.
  - **Neo4j / Memgraph / FalkorDB** — one shared pattern builder, `graflo.db.cypher.cypher_rel_pattern`, renders `-[r]->` / `<-[r]-` / `-[r]-`. Direction is purely a rendering question on this family, which is why all three share it.
  - **Nebula** — `GO … REVERSELY` and `BIDIRECT` are now emitted; both clauses previously had **zero occurrences** in the codebase, despite Nebula storing an out-key and an in-key per edge. `$$` is the far end of whichever orientation was walked, so the other-endpoint predicate needs no special-casing.
  - **TigerGraph raises rather than under-reports.** Reverse reachability there is fixed when the edge type is created, so an `IN` / `ANY` read of a directed type with no paired reverse type raises `UnsupportedEdgeDirectionError`; an `UNDIRECTED` type answers both ways natively, and a declared `reverse_edge` is queried as the reverse orientation. Returning only the outgoing half would hand back a partial neighbourhood with no signal — the one case where silence is worse than failure.
  - **PostgreSQL** — `define_edge_indexes` was a `pass`; it now creates a `target_id` index per edge table. The only pre-existing edge index is the composite uniqueness constraint, whose leading column is `source_id`, so it cannot serve a lookup keyed on the target. `fetch_edges` itself is still unimplemented there.
  - Verified end-to-end against live ArangoDB, Neo4j, Memgraph, FalkorDB and Nebula: an edge ingested `P1 -> P2` with `directed: false` is reachable from `P2`. The suite asserts the control too — `OUT` from `P2` must find nothing, or an `ANY` match would prove only that the query is indiscriminate.
...
- **Meta-ontology `1.3.0`** — `gf:edgeDirected` (datatype, boolean, domain `gf:Edge`). Direction is now a first-class predicate alongside `gf:edgeType` / `gf:edgeBy` rather than a key inside the opaque `gf:edgePayload` blob — it is part of the edge contract, not an extension field. Emitted only when false, since `true` is the default and the round-trip is asserted on the model; `gf:edgePayload` is still *read* on load, so graphs serialized before the predicate existed keep deserializing. Mirrored in `graflo-context.jsonld` and `rdf/namespace.py`; `docs/assets/graflo-ontology-viz/` regenerated. The round-trip had no test at all before this.

### Changed

- **Ontology visualization is now hierarchical and deterministic** (`docs/_build/scripts/ontology_viz/`, regenerated into `docs/assets/graflo-ontology-viz/`). The old view seeded a force simulation with `Math.random()`, so the picture differed on every page load, and `gf:GrafloArtifact`'s 28 children were laid out as one ~5400px-wide row that `fitToScreen` shrank to 0.24 scale. Replaced with left-to-right ranked columns inside labelled blocks, with no overlap resolution because the layout cannot overlap by construction. Fit scale in the 1400x720 embed goes 0.24 -> ~0.66.
  - **Columns are ranked by composition, not only by taxonomy.** Suppressing `gf:GrafloArtifact` leaves the Schema block with no internal `subClassOf` edge whatsoever, so ranking on taxonomy alone dropped its 17 classes into an arbitrary grid while Ingestion and Bindings read general-to-specific. A class now ranks one column right of whatever *contains* it — an object property's range sits right of its domain — as well as right of whatever generalises it. Distance from the block root is the shortest one, so peers stay peers (`gf:Vertex` and `gf:Edge` share a column despite the `gf:edgeSource` reference between them), then taxonomy is relaxed to a fixpoint so a subclass is never level with its superclass. `test_build_ontology_viz.py` restates the rule in Python and checks it against the payload.
  - **Classes are grouped into the blocks the manifest composes**, and the grouping is *derived*, not curated — no class-to-block map exists anywhere in the code. `extract.py` finds the universal root by intersecting `subClassOf` ancestor closures, finds the entry class (`gf:GraphManifest`) as the object-property domain that is never a range and whose only own superclass is that root, and takes its object-property ranges as the block seeds (`gf:Schema`, `gf:IngestionModel`, `gf:Bindings`). BFS from those seeds partitions all 46 classes. Adding a block to the ontology adds a band to the viz with no code change; if any rule stops yielding a unique answer the extractor degrades to a single ungrouped band and a test names the rule that broke.
  - **`gf:GrafloArtifact` is no longer drawn by default.** It is a superclass of 28 of 46 classes, so its arrows say nothing while hiding the three real sub-taxonomies (`gf:Actor`, `gf:BoundConnector`, `gf:ProtoTransform`). Which class to suppress is computed, not named. It stays in `graflo.ttl` and in `graph-data.json` — suppression is a rendering decision, reversible with a **Show GrafloArtifact** toggle, and the details panel still reports it in the ancestry breadcrumb.
  - Taxonomy-only is the default filter; selecting a class reveals its own properties. Each `subClassOf` edge is drawn once — the dashed reverse duplicate carried no information and doubled the taxonomy ink.
  - The viewer follows the docs theme: `prefers-color-scheme` plus a `?scheme=` parameter and a `postMessage` handler, wired from `docs/concepts/schema/ontology.md` to mkdocs-material's palette toggle. The embed previously stayed white under the `slate` scheme.

### Fixed

- **`ManifestRdfSerializer` raised `KeyError` on any manifest using an API or Kafka connector.** `_emit_connector` looks the model up in `ns.CONNECTOR_CLASSES`, which only ever held the three original connectors, so `to_graph`/`to_turtle`/`to_json_ld` crashed outright; `BOUND_SOURCE_KIND_INDIVIDUALS` was also missing `api`/`kafka`, and the deserializer's unknown-type fallback would have silently rebuilt either one as a `FileConnector`. All five kinds now round-trip, and two drift guards enumerate `ResourceConnector.__subclasses__()` and `BoundSourceKind` against the ontology and the registries, so the next connector fails in CI instead of at a user's serialization call.
- **All 55 datatype properties were silently dropped from the ontology viz.** They were emitted as class-to-class edges, which requires the range to be a rendered node — but every datatype property ranges over `xsd:*`, so not one survived, and the marker, CSS rule and legend entry for them were unreachable. They are now attached to their `rdfs:domain` class (and the 10 domain-less ones — `gf:name`, `gf:version`, `gf:description`, ...) surface as payload-level `sharedProperties`. A test asserts no declared datatype property goes missing.
- **`gf:BoundSourceKind`, `gf:TransformTarget` and `gf:TransformStrategy` were misclassified** as ordinary classes in the viz, because enumerations were detected by a `endswith("Type"|"Mode"|"Policy")` name heuristic. Detection is now structural — a class is an enumeration iff the ontology declares individuals of it — and their values are listed in the details panel.
- **Version drift in `docs/concepts/schema/ontology.md`** — the identifier table still advertised ontology version `1.0.0` against a shipped `1.3.0`. Fixed, and a test now asserts the page names the current `GF_VERSION`; a second asserts every `gf:` term the prose block-list mentions still exists in the ontology.
- **Memgraph `fetch_edges` crashed on every call** — it read column names as `desc[0]`, but `mgclient` yields `Column` objects that expose `.name` and are not subscriptable, so the method raised `TypeError` before returning a row. Nothing exercised it until the direction suite did. Column-name extraction now branches explicitly on the two driver shapes and raises a named error on a third, rather than defaulting its way into a `TypeError` several frames from the cause.
