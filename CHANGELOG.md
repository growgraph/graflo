...
- **Equivalence-cluster consistency for compose.** `VertexEquivalence` edges are treated as a bipartite graph (`graflo.architecture.evolution.equivalence`); connected components must agree on one `into`. Overlapping declarations that previously last-write-won in a rename dict now raise `ClusterConflictError`. `validate_and_complete_canonical_map` accepts multi-side canonical maps, completes unmapped peers along resolved clusters, and replaces the left-only `into == left` rule. `ComposeManifestsOp.identity_alignments` folds post-compose identity alignment into the compose step. Example 19 covers an n-ary cluster and a conflicting-cluster demo.
