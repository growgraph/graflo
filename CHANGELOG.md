...
- **Every refusal the schema union raises is now a finding.** `preview_compose` runs the real merge kernel per cluster — `merge_vertex_models`, and `merge_edge_pair` over both sides' edges folded by `edge_id` — so a field type clash, a unit clash, two identity modes that exclude each other, a divergent identity funnel, a secondary identity claimed twice and an edge whose declarations disagree on `type`/`by` are all reported by the code that decides them, rather than by a parallel implementation that can drift. New finding kinds: `unit_conflict`, `identity_mode_conflict`, `identity_funnel_conflict`, `secondary_identity_conflict`, `edge_conflict`.
- **`Refusal`** (`architecture/refusal.py`) — the base every compose-time refusal now shares, carrying `check` (the rule that refused) and `subjects` (the names it is about). It replaces five identical copies of the same `__init__` and gives the union's refusals the same structure the canonical-map ones already had: `FieldMergeError`, `VertexMergeError` and `EdgeMergeError` name their rule, so a caller can classify a refusal without parsing its prose. Still a `ValueError`, so every existing handler keeps catching it.
- **`relabel_vertex_fields`** (`architecture/evolution/apply.py`) — one vertex with its attribute names rewritten, as a new validated model. Extracted from `_rename_fields_in_schema`, which now calls it; it is what lets the preview reach the union's inputs without running a whole-side canonicalize.
...
- **Structured refusals.** `ComposeCanonicalConflictError`, `ComposeIncompleteError`, `ComposeNameConflictError`, `ComposeIdentityError` and `ClusterConflictError` now carry `check` (the rule that refused) and `subjects` (the names it is about, as `subject()` ids such as `left:Firm` or `left:Firm.firm_id`). A new `UnknownMemberError` replaces the bare `ValueError` from `check_member_existence`, so a caller classifying refusals can key on the type. All five now derive from the shared `Refusal` base above. All keyword-only with defaults, and no message text changed.
...
- `merge_field_lists` raises `FieldMergeError` rather than a bare `ValueError`, carrying every conflicting property as structured data. Messages are byte-identical, and `check` is derived from the same predicate that picks the heading, so the sentence an author reads and the rule a caller keys on cannot disagree.
- `graflo merge --plot` / `--plot-history` and the compose findings table now have tests. The table is asserted through the CLI as well as on the formatter: it goes to **stderr** on a refused run, which is why reading `result.stdout` had looked like a capture problem.
...
- **`blank` and `hash` were mutually exclusive everywhere except in the code.** Neither `Vertex.set_identity` nor `merge_vertex_models` refused the pair, and `identity_mode` tests `blank` first — so a vertex carrying both, or a union of a blank source with a hash-identity one, validated cleanly and then keyed on a generated id with the declared digest never consulted. Rows that should have deduplicated did not, and nothing said so. Both sides now refuse it.
- **The compose preview was blind to everything the schema union refuses.** Those refusals were bare `ValueError`s carrying no `check`, so `expected_kinds()` classified them to nothing and the consistency invariant *skipped* the case instead of failing it — which is why a green suite showed no sign of it. The invariant now **fails closed**: a refusal it cannot classify fails, and the message says what to do about it.
- **A refusal the preview did anticipate was still double-reported.** A field type clash produced both a `possible type_conflict` and a `refusal disagreement`; the fold that collapses the two requires matching kinds, and the untyped refusal could not supply one. One problem now renders as one row.
- **`_check_attribute_fixed_points` never fired for a member the canonical map renames.** It looked the fixed-point set up by member name where `canonical_property_names` keys on the *canonical class*, so for exactly the members the check exists to guard, the set came back empty and a canonical attribute could be re-targeted with nothing objecting. The only test covering the rule used an unrenamed member, where either lookup works.
- **Cross-resource identity alignments were not closed transitively.** `_canonical_field_maps` chose a canonical name per *pair*, so three resources describing one column under three spellings (`a <-> b`, `b <-> c`) ended up disagreeing, the shared-field intersection came back empty, and discovery fell back to `no_viable_identity` — adding a third source that carried the very same key made the key undiscoverable. Alignments are now closed into connected components and named over the whole group. A component holding two columns of one resource is ambiguous — projecting both onto one name would have silently dropped one — and is reported under `evidence["ambiguous_alignments"]` instead.
- **`apply_proposal_to_vertex` never wrote `blank`.** A proposal states the whole identity policy, but `blank` was left as the vertex had it, so applying a hash proposal to a blank vertex was accepted and then ignored.
...
- **Synthesized clusters.** Under `name_conflict="union_right"` every name both sides carry after their composite maps, and every pair of spellings that key alike, becomes a 1-1 equivalence into the left spelling, so a union by name goes through the same identity and property reconciliation as a declared one. `Cluster.synthesized` marks them; `Cluster.declared_into` keeps the composed name as the author spelled it.
- `canonicalize_ops`, `canonical_near_collisions`, `DeclaredMaps` and `Completion` on `graflo.architecture.evolution`.
- **Two incompleteness demos in example 19**, one per `Completion.kind`: `--forgotten-member-demo` leaves a member out of the cluster while the map still sends it to the composed class, and `--shared-name-demo` lets both sides arrive at a name no cluster composes (`--union-right` unions it instead of refusing). Both print their completion as YAML, as `graflo compose` does.
- **One vocabulary for compose resolution** — declared map, cluster, cluster map, composite map, fixed point, opinion, satisfied entry, dangling entry, synthesized cluster, completion — stated identically in `canonical.py` and the evolution guide, with the table of every case a declared map and the equivalences can stand in.
...
- **`name_conflict="fuse_right"` is now `"union_right"`** (`fuse_right` still parses, and is normalized on load). `fuse` names *records* becoming one node everywhere else in the contract (`allow_observation_fusion`, identity alignment); this policy unions two type *names*, so it takes the other verb. Compose is excluded from the revision vocabulary, so no stored change set carries the old spelling. It also **unions by name** now: it previously fused a near collision (`Deal` / `deal`) but refused an exact one (`Deal` / `Deal`), inverting its own principle that a weaker signal must not act more aggressively than a stronger one. Both now become synthesized clusters.
...
- **The canonical vocabulary and case table did not render in the API reference.** `canonical.py`'s module docstring used RST simple-table and definition-list syntax, which the Markdown docstring pipeline collapsed into run-on paragraphs. Both are Markdown now, and the exhaustive per-case table lives once, in the evolution guide.
- **`union_right` (then `fuse_right`) dropped the right vertex model.** After adopting the left spelling the right vertex was not a cluster, so the schema union skipped it: its properties and identity were lost and nothing raised. The union now refuses to skip any non-cluster vertex that shares a name with it.
...
- **`CanonicalizeOp`.** Applies a whole vocabulary map (classes, per-class attributes, relations) in one step over the original schema. Chains (`{X: Z, Z: Q}`) and swaps resolve without an intermediate state; the fibers of the map are the merge groups (`allow_merges`); an existing target that does not move must be declared with a self entry (`Company: Company`). Every refusal is computed before anything is rewritten. Reversible when it only renames. The op vocabulary grows to 38.
- **`ComposeManifestsOp.canonical_maps`**, scoped `left` / `right` / `both`, so a compose recipe is one document.
- **`into` is optional on `VertexEquivalence` / `RelationEquivalence`.** The composed name is `into` (translated through the map), else the canonical name the map gives a member, else the one spelling all members share; otherwise the cluster is refused as unnamed. A member may be spelled by its own or its canonical name — in the equivalence, its property equivalences, `SideIdentity.members` and identity-alignment member keys alike.
- **`resolve_clusters`** → `ClusterResolution` (the resolved clusters plus one composite `CanonicalMap` per side), with `ClusterSpec` as the resolved shape of one declaration and `check_member_existence` in `equivalence`.
- **`add_resources` / `remove_resources`.** Add full `ResourceConfig` definitions; remove by name, pruning the `resource_connector` entries of a removed resource. Inverses of each other, unless a removal pruned a binding. `diff_manifests` emits them instead of reporting the resource as inexpressible.
- **`remove_edges` accepts edge triples** (`edges: [{source, target, relation}]`) alongside `relations` — removes one endpoint pair, and is the only way to remove an edge with no relation.
- **`add_edge_properties` accepts a full `Field`**, as `add_vertex_properties` does, and rejects an unknown relation.
- **`set_field_semantics` reaches edge properties** through `EdgeFieldSemanticsTarget` (`source` / `target` / `relation` + `field`), with the same `FieldSemantics` and `unit` as a vertex property.
- **`diff_manifests` emits `set_vertex_semantics`, `set_edge_semantics`, `set_field_semantics` and edge-side `change_field_types`**, and warns on `db_profile` differences no op expresses. Added properties replay with their type and grounding, not as bare names.
- **`graflo rehash --store <dir>`** recomputes every commit id of a stored history in topological order (`--dry` reports first). See *Op fields renamed* below.

### Changed

- **Relicensed from the Business Source License 1.1 to Apache License 2.0.** No production-use conditions, no change date; earlier releases keep the BSL terms they shipped with. A `NOTICE` file carries the copyright line and grants no rights in the GraFlo and GrowGraph marks. The licence is declared as a PEP 639 SPDX expression; `CITATION.cff` follows.
- **Compose applies its canonical maps itself.** Clusters and maps resolve into one composite map per side, applied in a single `CanonicalizeOp` before the union. Equivalences no longer have to be written in a pre-canonicalized vocabulary. A map entry the caller already applied (source absent, target present) is a no-op, so callers that canonicalize a side first keep working.
- **One consistency rule between canonical maps and equivalences:** both must agree on where every name goes, and a canonical target is a fixed point neither may re-map. `validate_and_complete_canonical_map` refuses a disagreement (naming both declarations), an unnamed cluster, a map entry merging a non-member into a composed class, a dangling map entry, and a property equivalence naming a field the member does not have as spelled.
- **`canonical_map_to_ops` returns a single `CanonicalizeOp`** instead of a property-rename / merge / rename sequence.
- **`graflo compose` no longer canonicalizes each side before composing.** `--canonical-map` accepts `both` and is folded into the op.
- **Op fields renamed for what they hold.** `rename_vertices.vertices`, `rename_relations.relations`, `rename_resources.resources` → `renames`; `add_inverse_edges.relations` → `inverses`; `replace_identity.vertices` → `replacements`; `compose_manifests.vertices` / `.relations` → `vertex_equivalences` / `relation_equivalences`. Old keys still parse; new keys serialize and appear in the JSON schema.
  - **Migration:** commit ids are derived from serialized ops, so a history containing a renamed op re-hashes. Run `graflo rehash --store <dir>` once per history written before this release; `graflo verify` reports the mismatch until then. Trees are unaffected.
- **Edge slots nest under their relation** (`relation/knows/edge/person/company`). Recorded `MergeRecipe` resolutions addressed by the old `edge/…` slot are reported as not needed on re-merge.
- **Stricter op validation at parse time.** Rename, removal, addition and inverse maps must be non-empty; `merge_vertices` / `merge_edges` reject `into` among `sources` and repeated sources; `add_inverse_edges` rejects two relations sharing one inverse and a relation named as its own inverse; index lists must be non-empty and edge index entries unique; edge selections must be unique. `remove_vertex_properties` guards every identity plane (hash properties, funnel branches, secondary identities); `change_field_types` refuses `LIST` on an edge identity token.
- **A property declared twice with two descriptions keeps both**, joined by a blank line. Previously the first won. Manifest-hash-visible.
- **Edges fold duplicate properties**, as vertices already did. Manifest-hash-visible.
- **Cluster API.** `Cluster` is generic over its declaration (`RelationCluster` is an alias) and carries resolved member names plus the author's spellings (`resolved`, `property_maps`). `index_clusters` takes resolved `ClusterSpec`s. `ClusterIndex.relation_labels` is a property. `merge_canonical_maps` reconciles author maps only. `CanonicalMap` is defined in `ops` and still importable from `canonical`.
- **Example 19** declares its cluster in each manifest's own vocabulary with no `into`, ships the `boundary_op.yaml` its README cites, and replaces `--stale-demo` with `--disagreeing-map-demo`.

### Removed

- **Stale-name refusals** (`stale class name`, `stale relation name`, `stale property name`) and `CanonicalMap.stale_class_names` / `stale_relation_names` / `stale_property_names`. A retired name in an equivalence is now translated; one the map sends elsewhere is a disagreement.
- **The "merges are lowered before renames" refusal** of the cluster index — the lowering is atomic.
- **`ClusterIndex.cluster_for`**, and the `index` keyword of `validate_and_complete_canonical_map`.

### Fixed

- **A canonical map `{X: Z, X2: Z, Z: Q}` merged `X` and `X2` into the old `Z`, then carried all three to `Q`.** Merges were emitted before renames, and no fixed op order is correct for every map. `Z` is now built from `X` and `X2`, `Q` from the old `Z` alone.
- **An equivalence `into` occupied by a class another declaration renames away** no longer raises, for a single member or a merge alike. The message for a genuinely occupied `into` no longer suggests a remedy the overlap check refuses.
- **`rename_relations` reported a false collision on a chain over one endpoint pair.** It now checks surviving edge ids only, as the vertex guard does.
- **The merge kernel dropped property `item_type` and `semantics`.** Two identical `LIST<STRING>` properties could not be fused, and `LIST<STRING>` vs `LIST<INT>` failed with a misleading pydantic error instead of naming the element types.
- **Every merge dropped class-level `Vertex.semantics` / `Edge.semantics`.**
- **Field merging had three policies; it now has one** (`merge_fields` / `merge_field_lists`) for vertices, edges and the merge path. `type` and `item_type` compare as a unit; descriptions and grounding union; a conflicting type, element type or `unit` raises, naming the class, property, both values and the remedy. All conflicts on one class are reported in one error.
- **`compose_manifests` took `db_flavor`, `target_namespace` and `default_property_values` from the left** even when only the right declared them. Each is now folded by what a side actually declared. Vertex and edge-spec indexes union on their full definition and raise on a conflicting `unique` / `type` / `sparse`; a conflicting `relation_name` on one physical edge raises.
- **`LIST<STRING>` → `LIST<INT>` produced no migration and no op.** `migrate.diff` and `evolution.autogenerate` now compare the whole type spec, on vertices and relations, and emitted payloads carry `item_type`.
- **`add_vertex_properties` / `add_edge_properties` silently ignored a redeclaration that changes a property.** It now raises, naming `change_field_types` / `set_field_semantics`; an identical restatement is still a no-op.
- **`diff_manifests` crashed on any manifest with edge physical specs**, and with it `merge_three_way`, `build_merge_commit` and `invert_ops`. Edge index diffs key on the full physical key, so a `purpose` variant is addressed as itself.
- **`compose_manifests` hung on a connector name collision under `prefix_right`** when the name already started with `r_`. Connectors now use ordinal disambiguation; `fuse_right` rejects the collision as documented.
- **Inverting `remove_secondary_identities` failed on a field-list selector** and ignored the `secondary` shorthand.
- **Inverses of additive ops removed entries that were already there.** They now undo only what their op added — by triple for edges — or refuse. An op that changed nothing no longer reports a missing inverse.
- **Three-way merge missed relation-vs-edge collisions** (removing a relation on one branch, flipping one of its edges on the other).
- **Compose and vertex merge dropped or duplicated silently:** a shared vertex index was emitted twice; a demoted composite key lost the author's column order; a reused secondary-identity name escaped the conflict check when one entry equalled the merged primary; a field-set with two names lost the second; two edges with one id but different `type` / `by` merged to the left's; duplicate filters compounded across the two merge passes.
- **Edge property diffs accumulated across sibling edges of one relation.** A relation-level op is emitted only when the siblings agree; the rest, and changes on a relation-less edge, are reported.
- **Edge properties lost `item_type` and `semantics` at parse.**
- **`replace_edge_identities` had no inverse** (now restored from the pre-state keys when the new keys add no property); **`ensure_extracted_fields`** is declared irreversible with a reason.
- **`retarget_edges`, `rename_edge_properties` and `remove_edge_properties` were classified as schema-only**, so the schema-artifact guard let them through with their ingestion half dropped.
...
- **Primitive cards for all GraFlo building blocks.** Every core primitive — `Vertex`, `Edge`, `ResourceConfig`, `Transform`, `AnyConnector`, `DatabaseProfile`, `GraphManifest` — now has a lightweight, declarative card summary (`VertexCard`, `EdgeCard`, `ResourceCard`, `TransformCard`, `ConnectorCard`, `DatabaseProfileCard`, `ManifestCard`). All cards inherit from a common `BaseCard` with an optional `id` field (for registry integration) and automatic `estimated_tokens` calculation. The existing `SchemaCard` was refactored to inherit from `BaseCard`, gaining the `id` field while preserving backward compatibility. Corresponding `build_*_card` builder functions are exported from `graflo.architecture.schema.context`.
- **The equivalence cluster is the declaration, and compose lowers it through `CanonicalMap`.** `VertexEquivalence.left` / `.right` (and `RelationEquivalence`'s) now take a class name **or a list**, so `{Company, Shop} ~ {Org, Branch} -> Company` is one declaration rather than three edges whose connected component had to be re-derived. The declaration *is* the cluster, which makes the three ways declarations can contradict each other checkable — a class **claimed by two** declarations, two declarations **sharing one `into`**, and an `into` that already names an **existing non-member class** on a side (previously a silent merge into an unrelated type) all raise `ClusterConflictError` before any rename. `PropertyEquivalence.left` / `.right` accept a per-member map (`{"Company": "company_key", "Shop": "shop_key"}`) or a bare name applying to every member; the previous per-side dict kept only the last edge's map.

  **Compose no longer resolves renames and merges itself.** Each declared cluster is lowered to a per-side `CanonicalMap` (`clusters_to_side_maps`) and applied with `canonical_map_to_ops` — the same lowering a hand-authored canonical map goes through — so one code path owns "which classes merge, which rename, in what order". `merge_canonical_maps` is the single conflict primitive underneath: a partial-function union where every source maps to one target and a **target of the base map is a fixed point**, used both to reconcile two author maps for one side and to check an author map against the lowered cluster. `CanonicalMap` gained `relations`, so a canonical vocabulary covers classes, attributes and relation names alike. `validate_and_complete_canonical_map` is side-aware throughout (a left `Org` is no longer confused with a right `Org`, and `pe.right` is checked against right-side maps) and returns `SideMaps` — the per-side maps the clusters lower to, which is what compose applies.

  **Merge acknowledgement is one rule at one layer.** `ComposeManifestsOp` gained `allow_merges` (required when a cluster names more than one member on a side), `allow_self_relations` and `allow_row_fusion`; compose forwards the latter two to the per-side `MergeVerticesOp` instead of forcing them on and downgrading the self-relation guard to a warning. The validator's `allow_implicit_merge` and the left-only `validate_compose_against_canonical_map` wrapper are gone.

  **The composed identity is declared, not guessed.** Members that disagree on their identity field-set after alignment used to compose into the union of both — a natural key no record carries. That now raises `ComposeIdentityError` naming each member's key, unless the cluster declares `identity` (a natural key, an `IdentityFunnel`, or a `SideIdentity` shorthand — per-side or per-member branch chains merged into one global funnel order, with an order inversion between members refused), a `PropertyEquivalence(identity=True)` flag resolves it, or an `identity_alignments` entry replaces it. A declared `identity` demotes each member's retired key to a lookup-only secondary identity unless `retire: keep`.

  Also: `ComposeManifestsOp.identity_alignments` is typed (`list[IdentityAlignment]`) rather than `list[Any]` behind an `object.__setattr__` validator — the alignment declaration models moved into `ops.py` beside the equivalences, re-exported from `alignment.py`; `compose_manifests(canonical_maps=...)` takes side-tagged `(side, CanonicalMap)` pairs; and `apply_rename_relations` gained the edge-id collision guard vertices already had. Example 19 is one n-ary declaration, and its conflicting-cluster demo authors an overlap.
