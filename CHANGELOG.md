...
- **The member is the unit of identity derivation.** An equivalence cluster names its members — the classes it collapses, per side — and every record that becomes the canonical class was produced *as one member* by *one resource*: a `vertex: Shop` step, or a `vertex_router` key whose value was `Shop`. Identity alignment could not say that. `AlignmentAttribute.sources` spoke only per resource in raw columns, so a resource routing two members onto the class could tell them apart only by column presence (each member keying from its own column) or by a hand-written `gate="kind", gate_prefix="shop"` that restated the router's own test with a different, prefix-based, semantics. When the members share one key column, each under its own marker, neither works: a value-only test strips the marker and fuses a `Shop` carrying the firm marker with the firm.

  `sources[resource]` — on `AlignmentAttribute` and `LocalKeySpec` alike — now also accepts a **dict keyed by member class**, alongside the single spec and the list. The lowering resolves each `(resource, member)` against the pre-merge *sides*, which `compose_manifests` now captures after canonical maps and the resource rename policy but before the cluster lowering — the merge rewrites router `type_map` values to the canonical name, after which no manifest can say which key produced which member. A member produced by a plain `vertex` step gets no gate (the level *is* the member); one produced by a router gets a `when` guard on the router's discriminator, exact match, listing the keys that mapped to it. The gate is derived, never written. Validation names a member the resource does not produce (listing what it does), a member outside the cluster on that side, members resolving to different levels, and member keys used without sides; partial coverage of a resource's members is warned about, and a member-keyed `LocalKeySource` may not set `gate`. The list form stays for members that key from their own columns, and `gate` / `gate_prefix` stay for an alignment applied outside a compose. **Example 21** now aligns on a column every kind of the view carries, with per-member markers, and shows a shop row carrying the firm marker falling through instead of fusing.

- **Conditional transform steps: `when`.** A transform step may carry a guard, `when: {field: kind, in: [shop]}`, and runs only when the observation's `field` holds one of the listed values — compared exactly, as a `vertex_router` compares its `type_map` keys. The property that matters is that a step whose guard fails **writes nothing**: no output, no `None`. A function returning `None` cannot offer that, and the difference shows behind a router, which merges the transform buffer into one observation dict where a later `None` overwrites an earlier real value. Under `when`, several steps may share one output field and each is the single writer for the documents it admits — which is what lets a member-keyed derivation write its attribute directly, with no scratch fields and no `coalesce_fields`. Those remain the lowering of the list form, where the branch is selected by a function declining with `None`.
