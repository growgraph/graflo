"""Draw a compose preview: the declaration graph, with its conflicts marked.

The picture a
:class:`~graflo.architecture.evolution.preview.ComposePreview` is asking to be:
each side's classes down its own column, the composed names between them, and
...
says *where* as well as *what*. A refusal — the one compose actually raised —
...
:func:`plot_compose_preview` is the half that writes a file.
"""
...
#: How a flagged element is drawn. A refusal is what compose raised; a
#: possible finding is what the preview found on its own and compose has not
#: reached yet.
...
def _numbered(preview: ComposePreview) -> list[ComposeFinding]:
...
def _legend(preview: ComposePreview) -> str:
...
def build_preview_graph(
    preview: ComposePreview, *, max_rows: int = 12, legend: bool = True
) -> nx.MultiDiGraph:
    """The preview as a networkx graph, styled but not yet drawn.

    Only classes, relations and composed names become nodes: attributes are
    ...
    Args:
        preview: What :func:`~graflo.architecture.evolution.preview.preview_compose` returned.
...
def _title(preview: ComposePreview) -> str:
...
def _columns(graph: nx.MultiDiGraph, agraph: Any) -> None:
    ...
    for name, data in graph.nodes(data=True):
        ...
        key = side or ("composed" if data.get("kind") == "composed" else "")
        if key:
            groups.setdefault(key, []).append(name)
    for key, label in (("left", "left"), ("composed", "composed"), ("right", "right")):
...
def plot_compose_preview(
    ...
    *,
    ...
    Args:
        preview: What ``preview_compose`` returned.
