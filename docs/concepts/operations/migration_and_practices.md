...
12. **Bidirectional edges**: declare the pair in `edge_config.inverses`, then choose one realization — (a) explicit inverse edges (`AddInverseEdgesOp`) for portability; (b) a TigerGraph native inverse (`db_profile.native_inverses`, `SetNativeInversesOp`) for a `WITH REVERSE_EDGE` pair with one load path — or (c) for truly symmetric relationships, `directed: false` edges, optionally declared in `edge_config.symmetric` (`UNDIRECTED EDGE` on TigerGraph). The schema refuses (a) and (b) on the same relation.
