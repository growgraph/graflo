...
12. **Bidirectional edges**: declare the pair in `edge_config.inverses`, then choose one realization — (a) an explicit inverse edge (`AddInverseEdgesOp`) for portability; (b) a TigerGraph native inverse (`edge_specs[*].native_inverse`, `SetNativeInversesOp`) for a `WITH REVERSE_EDGE` pair with one load path — or (c) `directed: false` for truly symmetric relationships (`UNDIRECTED EDGE` on TigerGraph). The schema refuses (a) and (b) on the same forward edge.
