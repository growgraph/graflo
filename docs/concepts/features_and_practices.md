...
- **Graph export & migration** — introspect Neo4j or ArangoDB (`introspect_graph_schema`), export **`GraFloOutput`**, or migrate graph→graph / graph→PostgreSQL via **`GraphEngine.export_graph()`** / **`migrate_graph()`**. See [Graph export and migration](graph_export_migration.md).
