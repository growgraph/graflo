...
| **Rename vertex fields** | Per-vertex `{old_field: new_field}` maps: updates schema field names, identities, `db_profile` index specs, and ingestion (`vertex` `from`, `transform.rename` targets) so documents still use the **source** column names where a reverse map is injected. Rejects an unknown source, and a rename onto a name already declared there — that folds two properties into one and no rename back restores them; a fold the map spells out (`{a: c, b: c}`, or `{a: b, b: b}`) is applied. |
| **Remove vertex fields** | Removes vertex properties, prunes vertex/edge index references, and rewrites ingestion references (`from`, `keep_fields`, `vertex_weights`). Rejects a property the vertex does not declare. |
| **Add vertex fields** | Adds properties to existing vertices for schema enrichment and migration planning. |
| **Rename edge fields** | Per-relation edge property renames across schema edge properties/identities, `db_profile` edge indexes/defaults, and edge actor `properties` payloads. Rejects an unknown relation, a source none of its edges carries, and a rename onto a name an edge already declares (an explicit fold is applied, as for vertices). |
| **Remove edge fields** | Removes per-relation edge properties, prunes edge index/default references, and rewrites edge actor `properties`. Rejects an unknown relation and a property none of its edges carries. |
