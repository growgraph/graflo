...
- **Graph export and migration** — introspect Neo4j, ArangoDB or PostgreSQL (or a file backend) and export to any supported target; optional file backend for large exports. **`migrate_graph()` does not currently complete a move** — its write step fails for every pair. See [Graph export and migration](../operations/graph_export_migration.md) and [Graph DB migration guide](../../guides/graph_db_migration.md).
