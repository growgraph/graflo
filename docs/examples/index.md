...
11. **[Flat-row dynamic edges with `vertex_router`](example-11.md)** — Each CSV row encodes a complete `(source, target, relation)` tuple. Two `vertex_router` steps accumulate endpoint vertices into named slots keyed by `type_field`; a dynamic `edge` step resolves types from those slots.
