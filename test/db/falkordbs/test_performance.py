"""Performance and load tests for FalkorDB connector.
...
Run all performance tests::
    ...
    pytest test/db/falkordbs/test_performance.py -v -s

Run quick tests only (exclude slow markers)::

    pytest test/db/falkordbs/test_performance.py -v -k "not slow"

Run sustained load tests::

    pytest test/db/falkordbs/test_performance.py -v -k "sustained"
...
- test.db.falkordbs.test_functional : Functional correctness tests
...
class TestScalability:
    ...
    def test_insert_scaling(self, conn_conf, test_graph_name, clean_db):
        ...
        with ConnectionManager(connection_config=conn_conf) as db:
            ...
            # FalkorDB shows significant degradation with volume
            if results[0][1] > 0:
...
class TestSustainedLoad:
    ...
    def test_sustained_write_load(self, conn_conf, test_graph_name, clean_db):
        ...
        if len(window_avgs) >= 2:
            ...
            # Note: FalkorDB shows increased latency under sustained load
