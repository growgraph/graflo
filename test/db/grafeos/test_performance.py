"""Performance and load tests for Grafeo connector.
...
Run all performance tests::
    ...
    pytest test/db/grafeos/test_performance.py -v -s

Run quick tests only (exclude slow markers)::

    pytest test/db/grafeos/test_performance.py -v -k "not slow"

Run sustained load tests::

    pytest test/db/grafeos/test_performance.py -v -k "sustained"
...
- Results vary based on Grafeo configuration and available resources

See Also
--------
- graflo.db.grafeo.conn : Grafeo connector implementation
- test.db.grafeos.test_functional : Functional correctness tests
- test.db.grafeos.test_edge_cases : Technical edge case tests
"""
...
class TestScalability:
    ...
    def test_insert_scaling(self, conn_conf, test_graph_name, clean_db):
        ...
        with ConnectionManager(connection_config=conn_conf) as db:
            ...
            # Document degradation (informational, not a pass/fail criterion)
            # Grafeo shows significant degradation with volume
            if results[0][1] > 0:
                degradation = results[-1][1] / results[0][1]
                print(f"\n  Degradation: {(1 - degradation) * 100:.1f}%")
                # Note: Degradation is normal for graph databases
                # This test documents behavior, not used as strict criterion
...
class TestBatchSizing:
    ...
    def test_batch_size_memory_impact(self, conn_conf, test_graph_name, clean_db):
        """Mesure l'impact mémoire des différentes tailles de batch."""
        _ = clean_db
        import sys
...
class TestSustainedLoad:
    ...
    def test_sustained_write_load(self, conn_conf, test_graph_name, clean_db):
        ...
        if len(window_avgs) >= 2:
            ...
            # Note: Grafeo shows increased latency under sustained load
            # This is typical for databases accumulating operations
            if degradation > 5.0:
