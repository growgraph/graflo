"""Functional tests for the Grafeo connector.

Adapted from test/db/falkordbs/test_functional.py.

Tests realistic scenarios: composite keys, upsert semantics, relationship
...
Run all functional tests::
    ...
    pytest test/db/grafeos/test_functional.py -v

Run specific category::

    pytest test/db/grafeos/test_functional.py -v -k "DataCoherence"

Notes
-----
- Tests require a running Grafeo instance (see conftest.py fixtures)
...
- test.db.grafeos.test_performance : Performance and load tests
"""
...
class TestSchemaEvolution:
    ...
    def test_type_migration(self, conn_conf, test_graph_name, clean_db):
        ...
        Grafeo handles mixed types gracefully (coerces or skips non-numeric),
        so AVG on mixed string/int scores does not raise.
        """
        _ = clean_db
        with ConnectionManager(connection_config=conn_conf) as db:
            # Old format (string)
            ...
            # Grafeo handles mixed types gracefully
            result = db.aggregate(
...
class TestTransactionalRobustness:
    ...
    def test_no_unique_constraint(self, conn_conf, test_graph_name, clean_db):
        """Document lack of unique constraints in Grafeo."""
        _ = clean_db
        with ConnectionManager(connection_config=conn_conf) as db:
            # First document
            db.upsert_docs_batch(
