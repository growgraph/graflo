...
Unlike merge, ``merge_three_way`` already *returns* its conflicts, so there
...
from graflo.architecture.evolution.preview import Merge3Preview, build_merge3_preview
...
def test_a_preview_survives_a_round_trip_through_json(ancestor, branches):
    ...
    assert Merge3Preview.model_validate(json.loads(json.dumps(payload))).to_dict() == (
        payload
    )
